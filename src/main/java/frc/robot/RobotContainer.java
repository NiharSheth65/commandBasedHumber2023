...
public class RobotContainer {
  ...
  private void configureButtonBindings(){
    ...
    // BUTTON_A.onTrue(new intakeWinchCommand(WINCH_SUBSYSTEM, 0.5)); 
    // BUTTON_B.onTrue(new outtakeWinchCommand(WINCH_SUBSYSTEM, 0.5)); 

    // BUTTON_A.onTrue(new intakeWinchCommand(WINCH_SUBSYSTEM, 0.5));
    
    //runs winch out, outakes, winch in
    BUTTON_A.onTrue(new outtakeWinchCommand(WINCH_SUBSYSTEM, 0.5));
    // .andThen(new intakeCommand(INTAKE_SUBSYSTEM, -0.5))
    // .andThen(new intakeWinchCommand(WINCH_SUBSYSTEM, 0.5))); 
     
    //Runs winch out, Intakes till sensor hit, Winch in
    BUTTON_B.onTrue(new outtakeWinchCommand(WINCH_SUBSYSTEM, 0.5)
    .andThen(new intakeCommand(INTAKE_SUBSYSTEM, 0.5))
    .andThen(new intakeWinchCommand(WINCH_SUBSYSTEM, 0.5))); 

    BUTTON_x.onTrue(new intakeWinchCommand(WINCH_SUBSYSTEM, 0.5));
  ...
  private void defaultCommands(){
    ...
    PISTON_SUBSYSTEM.setDefaultCommand(new pistonShooterCommand(PISTON_SUBSYSTEM, 0));
    // INTAKE_SUBSYSTEM.setDefaultCommand(new defaultUpperIntakeCommand(INTAKE_SUBSYSTEM, 0.1));
    double right = joystick.getRawAxis(4); 
    SmartDashboard.putString("lt", "hellow"); 
    SmartDashboard.putNumber("rt", joystick.getRawAxis(4)); 
